...
LKH_EXTERN int AscentCandidates;           /* Number of candidate edges to be associated
                                   with each node during the ascent */
LKH_EXTERN int BackboneTrials;             /* Number of backbone trials in each run */
LKH_EXTERN int Backtracking;               /* Specifies whether backtracking is used for
                                   the first move in a sequence of moves */
LKH_EXTERN GainType BestCost;              /* Cost of the tour in BestTour */
LKH_EXTERN int* BestTour;                  /* Table containing best tour found */
LKH_EXTERN GainType BetterCost;            /* Cost of the tour stored in BetterTour */
LKH_EXTERN int* BetterTour;                /* Table containing the currently best tour
                                   in a run */
LKH_EXTERN int CacheMask;                  /* Mask for indexing the cache */
LKH_EXTERN int* CacheVal;                  /* Table of cached distances */
LKH_EXTERN int* CacheSig;                  /* Table of the signatures of cached
                                   distances */
LKH_EXTERN int CandidateFiles;             /* Number of CANDIDATE_FILEs */
LKH_EXTERN int* CostMatrix;                /* Cost matrix */
LKH_EXTERN int Dimension;                  /* Number of nodes in the problem */
LKH_EXTERN int DimensionSaved;             /* Saved value of Dimension */
LKH_EXTERN int EdgeFiles;                  /* Number of EDGE_FILEs */
LKH_EXTERN double Excess;                  /* Maximum alpha-value allowed for any
                                   candidate edge is set to Excess times the
                                   absolute value of the lower bound of a
                                   solution tour */
LKH_EXTERN int ExtraCandidates;            /* Number of extra neighbors to be added to
                                   the candidate set of each node */
LKH_EXTERN Node *FirstActive, *LastActive; /* First and last node in the list
                                   of "active" nodes */
LKH_EXTERN Node* FirstNode;                /* First node in the list of nodes */
LKH_EXTERN Segment* FirstSegment;          /* A pointer to the first segment in the cyclic
                                   list of segments */
LKH_EXTERN SSegment* FirstSSegment;        /* A pointer to the first super segment in
                                   the cyclic list of segments */
LKH_EXTERN int Gain23Used;                 /* Specifies whether Gain23 is used */
LKH_EXTERN int GainCriterionUsed;          /* Specifies whether L&K's gain criterion is
                                   used */
LKH_EXTERN double GridSize;                /* The grid size of toroidal instances */
LKH_EXTERN int GroupSize;                  /* Desired initial size of each segment */
LKH_EXTERN int SGroupSize;                 /* Desired initial size of each super segment */
LKH_EXTERN int Groups;                     /* Current number of segments */
LKH_EXTERN int SGroups;                    /* Current number of super segments */
LKH_EXTERN unsigned Hash;                  /* Hash value corresponding to the current tour */
LKH_EXTERN Node** Heap;                    /* Heap used for computing minimum spanning
                                   trees */
LKH_EXTERN HashTable* HTable;              /* Hash table used for storing tours */
LKH_EXTERN int InitialPeriod;              /* Length of the first period in the ascent */
LKH_EXTERN int InitialStepSize;            /* Initial step size used in the ascent */
LKH_EXTERN double InitialTourFraction;     /* Fraction of the initial tour to be
                                   constructed by INITIAL_TOUR_FILE edges */
LKH_EXTERN char* LastLine;                 /* Last input line */
LKH_EXTERN double LowerBound;              /* Lower bound found by the ascent */
LKH_EXTERN int Kicks;                      /* Specifies the number of K-swap-kicks */
LKH_EXTERN int KickType;                   /* Specifies K for a K-swap-kick */
LKH_EXTERN int M;                          /* The M-value is used when solving an ATSP-
                                   instance by transforming it to a STSP-instance */
LKH_EXTERN int MaxBreadth;                 /* The maximum number of candidate edges
                                   considered at each level of the search for
                                   a move */
LKH_EXTERN int MaxCandidates;              /* Maximum number of candidate edges to be
                                   associated with each node */
LKH_EXTERN int MaxMatrixDimension;         /* Maximum dimension for an explicit cost matrix */
LKH_EXTERN int MaxSwaps;                   /* Maximum number of swaps made during the
                                   search for a move */
LKH_EXTERN int MaxTrials;                  /* Maximum number of trials in each run */
LKH_EXTERN int MergeTourFiles;             /* Number of MERGE_TOUR_FILEs */
LKH_EXTERN int MoveType;                   /* Specifies the sequantial move type to be used
                                   in local search. A value K >= 2 signifies
                                   that a k-opt moves are tried for k <= K */
LKH_EXTERN Node* NodeSet;                  /* Array of all nodes */
LKH_EXTERN int Norm;                       /* Measure of a 1-tree's discrepancy from a tour */
LKH_EXTERN int NonsequentialMoveType;      /* Specifies the nonsequential move type to
                                   be used in local search. A value
                                   L >= 4 signifies that nonsequential
                                   l-opt moves are tried for l <= L */
LKH_EXTERN GainType Optimum;               /* Known optimal tour length.
                                   If StopAtOptimum is 1, a run will be
                                   terminated as soon as a tour length
                                   becomes equal this value */
LKH_EXTERN int PatchingA;                  /* Specifies the maximum number of alternating
                                   cycles to be used for patching disjunct cycles */
LKH_EXTERN int PatchingC;                  /* Specifies the maximum number of disjoint cycles to be
                                   patched (by one or more alternating cycles) */
LKH_EXTERN int Precision;                  /* Internal precision in the representation of
                                   transformed distances */
LKH_EXTERN int PredSucCostAvailable;       /* PredCost and SucCost are available */
LKH_EXTERN int POPMUSIC_InitialTour;       /* Specifies whether the first POPMUSIC tour
                                   is used as initial tour for LK */
LKH_EXTERN int POPMUSIC_MaxNeighbors;      /* Maximum number of nearest neighbors used
                                   as candidates in iterated 3-opt */
LKH_EXTERN int POPMUSIC_SampleSize;        /* The sample size */
LKH_EXTERN int POPMUSIC_Solutions;         /* Number of solutions to generate */
LKH_EXTERN int POPMUSIC_Trials;            /* Maximum trials used for iterated 3-opt */
LKH_EXTERN unsigned* Rand;                 /* Table of random values */
LKH_EXTERN int Recombination;              /* IPT or GPX2 */
LKH_EXTERN int RestrictedSearch;           /* Specifies whether the choice of the first
                                   edge to be broken is restricted */
LKH_EXTERN short Reversed;                 /* Boolean used to indicate whether a tour has
                                   been reversed */
LKH_EXTERN int Run;                        /* Current run number */
LKH_EXTERN int Runs;                       /* Total number of runs */
LKH_EXTERN unsigned Seed;                  /* Initial seed for random number generation */
LKH_EXTERN int StopAtOptimum;              /* Specifies whether a run will be terminated if
                                   the tour length becomes equal to Optimum */
LKH_EXTERN int Subgradient;                /* Specifies whether the Pi-values should be
                                   determined by subgradient optimization */
LKH_EXTERN int SubproblemSize;             /* Number of nodes in a subproblem */
LKH_EXTERN int SubsequentMoveType;         /* Specifies the move type to be used for all
                                   moves following the first move in a sequence
                                   of moves. The value K >= 2 signifies that a
                                   K-opt move is to be used */
LKH_EXTERN int SubsequentPatching;         /* Species whether patching is used for
                                   subsequent moves */
LKH_EXTERN SwapRecord* SwapStack;          /* Stack of SwapRecords */
LKH_EXTERN int Swaps;                      /* Number of swaps made during a tentative move */
LKH_EXTERN double TimeLimit;               /* The time limit in seconds for each run */
LKH_EXTERN int TraceLevel;                 /* Specifies the level of detail of the output
                                   given during the solution process.
                                   The value 0 signifies a minimum amount of
                                   output. The higher the value is the more
                                   information is given */
LKH_EXTERN int Trial;                      /* Ordinal number of the current trial */

/* The following variables are read by the functions ReadParameters and
   ReadProblem: */

LKH_EXTERN char *ParameterFileName, *ProblemFileName, *PiFileName, *TourFileName, *OutputTourFileName,
    *InputTourFileName, **CandidateFileName, **EdgeFileName, *InitialTourFileName,
    *SubproblemTourFileName, **MergeTourFileName;
LKH_EXTERN char *Name, *Type, *EdgeWeightType, *EdgeWeightFormat, *EdgeDataFormat, *NodeCoordType, *DisplayDataType;
LKH_EXTERN int CandidateSetSymmetric, CandidateSetType, CoordType, DelaunayPartitioning, DelaunayPure,
...
LKH_EXTERN FILE *ParameterFile, *ProblemFile, *PiFile, *InputTourFile, *TourFile, *InitialTourFile,
    *SubproblemTourFile, **MergeTourFile;
LKH_EXTERN CostFunction Distance, D, C, c;
LKH_EXTERN MoveFunction BestMove, BacktrackMove, BestSubsequentMove;
LKH_EXTERN MergeTourFunction MergeWithTour;
