...
void KinoReplanFSM::init(rclcpp::Node::SharedPtr node) {
  node_ = node;
  current_wp_ = 0;
  ...
  /* callback */
  exec_timer_ = node_->create_wall_timer(std::chrono::milliseconds(10), std::bind(&KinoReplanFSM::execFSMCallback, this));
  safety_timer_ = node_->create_wall_timer(std::chrono::milliseconds(50), std::bind(&KinoReplanFSM::checkCollisionCallback, this));

  waypoint_sub_ =
      node_->create_subscription<nav_msgs::msg::Path>("/waypoint_generator/waypoints", 1, std::bind(&KinoReplanFSM::waypointCallback, this, std::placeholders::_1));
...
bool KinoReplanFSM::callKinodynamicReplan() {
  ...
  rclcpp::Time time_r = node_->get_clock()->now();
  bool plan_success =
      planner_manager_->kinodynamicReplan(start_pt_, start_vel_, start_acc_, end_pt_, end_vel_);

  auto t2 = node_->get_clock()->now();
  if (plan_success) {
    replan_time_.push_back((t2 - t1).seconds());
    double mean1 = 0.0;
    for (auto t : replan_time_)
      mean1 += t;
    mean1 /= replan_time_.size();
    RCLCPP_WARN(node_->get_logger(), "Replan number: %ld, mean traj: %lf", replan_time_.size(), mean1);
  }
