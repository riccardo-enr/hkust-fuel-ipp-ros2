...
void FastExplorationManager::initialize(rclcpp::Node::SharedPtr node) {
  node_ = node;
  ...
  ep_->refined_radius_ = node_->get_parameter("exploration/refined_radius").as_double();
  ep_->top_view_num_ = node_->get_parameter("exploration/top_view_num").as_int();
  ep_->max_decay_ = node_->get_parameter("exploration/max_decay").as_double();
  ep_->tsp_dir_ = node_->get_parameter("exploration/tsp_dir").as_string();
  ep_->relax_time_ = node_->get_parameter("exploration/relax_time").as_double();

  ViewNode::vm_ = node_->get_parameter("exploration/vm").as_double();
  ViewNode::am_ = node_->get_parameter("exploration/am").as_double();
  ViewNode::yd_ = node_->get_parameter("exploration/yd").as_double();
  ViewNode::ydd_ = node_->get_parameter("exploration/ydd").as_double();
  ViewNode::w_dir_ = node_->get_parameter("exploration/w_dir").as_double();
...
int FastExplorationManager::planExploreMotion(
  ...
  RCLCPP_WARN(node_->get_logger(),
      "Frontier: %ld, t: %lf, viewpoint: %ld, t: %lf", ed_->frontiers_.size(), frontier_time,
  ...
  if (ed_->points_.size() > 1) {
    ...
    if (ep_->refine_local_) {
      ...
      RCLCPP_WARN(node_->get_logger(), "Local refine time: %lf", local_time);
  ...
  RCLCPP_WARN(node_->get_logger(), "Traj: %lf, yaw: %lf", traj_plan_time, yaw_time);
  double total = (node_->now() - t2).seconds();
  RCLCPP_WARN(node_->get_logger(), "Total time: %lf", total);
...
void FastExplorationManager::findGlobalTour(
  ...
  RCLCPP_WARN(node_->get_logger(), "Cost mat: %lf, TSP: %lf", mat_time, tsp_time);
}
