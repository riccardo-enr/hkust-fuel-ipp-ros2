...
public:
  ...
  void Update(nav_msgs::msg::OccupancyGrid m)
  {
    ...
    tf2::Quaternion q_tf;
    tf2::fromMsg(m.info.origin.orientation, q_tf);
    double oyaw = tf2::getYaw(q_tf);
  ...
  {
    // Note: ROS 2 doesn't use map_load_time in the same way
    // map.header.stamp and map.info.map_load_time would need to be set by caller if needed
    map.header.frame_id = string("map"); // ROS 2 convention: no leading slash
    updated = false;
    return map;
  }
};
