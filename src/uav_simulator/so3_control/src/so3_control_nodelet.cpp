...
class SO3ControlNodelet : public rclcpp::Node {
public:
  explicit SO3ControlNodelet(const rclcpp::NodeOptions & options)
    ...
    this->declare_parameter("mass", 0.74);
    ...
    odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "odom", 10, std::bind(&SO3ControlNodelet::odom_callback, this, std::placeholders::_1));
    position_cmd_sub_ = this->create_subscription<quadrotor_msgs::msg::PositionCommand>(
        "position_cmd", 10, std::bind(&SO3ControlNodelet::position_cmd_callback, this, std::placeholders::_1));

    enable_motors_sub_ = this->create_subscription<std_msgs::msg::Bool>(
        "motors", 2, std::bind(&SO3ControlNodelet::enable_motors_callback, this, std::placeholders::_1));
    corrections_sub_ = this->create_subscription<quadrotor_msgs::msg::Corrections>(
        "corrections", 10, std::bind(&SO3ControlNodelet::corrections_callback, this, std::placeholders::_1));

    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        "imu", 10, std::bind(&SO3ControlNodelet::imu_callback, this, std::placeholders::_1));
  }
...
void SO3ControlNodelet::publishSO3Command(void) {
  ...
  for (int i = 0; i < 3; i++) {
    so3_command.kr[i] = kR_[i]; // Changed to kr
    so3_command.kom[i] = kOm_[i]; // Changed to kom
  }
...
void SO3ControlNodelet::position_cmd_callback(const quadrotor_msgs::msg::PositionCommand::SharedPtr cmd) {
...
void SO3ControlNodelet::odom_callback(const nav_msgs::msg::Odometry::SharedPtr odom) {
...
void SO3ControlNodelet::enable_motors_callback(const std_msgs::msg::Bool::SharedPtr msg) {
  if (msg->data)
...
void SO3ControlNodelet::corrections_callback(const quadrotor_msgs::msg::Corrections::SharedPtr msg) {
  corrections_[0] = msg->kf_correction;
  corrections_[1] = msg->angle_corrections[0];
  corrections_[2] = msg->angle_corrections[1];
}

void SO3ControlNodelet::imu_callback(const sensor_msgs::msg::Imu::SharedPtr imu) {
  const Eigen::Vector3d acc(imu->linear_acceleration.x, imu->linear_acceleration.y,
                            imu->linear_acceleration.z);
  controller_.setAcc(acc);
}

RCLCPP_COMPONENTS_REGISTER_NODE(SO3ControlNodelet)