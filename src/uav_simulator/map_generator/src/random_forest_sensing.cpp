...
public:
  RandomForestSensing() : Node("random_map_sensing") {
    ...
    _odom_sub = this->create_subscription<nav_msgs::msg::Odometry>(
        "odometry", 50, std::bind(&RandomForestSensing::rcvOdometryCallbck, this, std::placeholders::_1));
    
    click_sub = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "/goal", 10, std::bind(&RandomForestSensing::clickCallback, this, std::placeholders::_1));
    ...
    timer_ = this->create_wall_timer(
      std::chrono::duration<double>(1.0 / _sense_rate),
...
private:
  ...
  uniform_real_distribution<double> rand_x, rand_y, rand_w, rand_h;
  uniform_real_distribution<double> rand_radius_, rand_radius2_, rand_theta_, rand_z_;
  ...
  rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr click_sub;
  ...
  double radius_l_, radius_h_, z_l_, z_h_, theta_;
